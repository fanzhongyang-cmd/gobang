...
public class GobangPanel extends javax.swing.JPanel {
	...
	public static void playVoice(){
		try{
			File f=new File("src\\res\\music\\chessVoice.wav");
	...
	public void paint(Graphics g1) {
		...
		if(difPoint!=null)
			g.drawImage(rightTop_img,difPoint.x_index*chessWidth,difPoint.y_index*chessHeight,chessWidth,chessHeight,this);
	...
	private void formMouseClicked(java.awt.event.MouseEvent evt) {
		...
		if (!start || !isTowardsStart() || myColor == 0 || !turn) {
			JOptionPane.showMessageDialog(this,"未开始");
			return;
		}
		...
		if (chessmanArray[xindex][yindex] == 0) {// 如果鼠标位置上没有棋子
			...
			difPoint=new point(xindex,yindex);
			repaint();// 重绘组件
	...
	private void winAction(){
		...
		UserBean user=mainFrame.getUser();
		user.winCountAdd();//wincount++
		int leng=0;
		for(int i=0;i<gobangModel1.getChessmanArray().length;i++){
			for(int j=0;j<gobangModel1.getChessmanArray().length;j++){
				if(gobangModel1.getChessmanArray()[i][j]!=0)
					leng++;
			}
		}
		user.setRank(user.getRank()+(int)leng/255*30);
		Connection con=null;
		Statement stmt=null;
		try{
			//连接数据库
			...
			String sql="update userinfo set userrank='"+user.getRank()+"',wincount='"+user.getWin_count()+"'where userid='"+user.getUserID()+"'";
			stmt.execute(sql);
		}catch (Exception ex){
			Logger.getLogger(LoginPanel.class.getName()).log(
	...
	private void loseAction(){
		...
		user.setRank(user.getRank()-30*(1-leng/255));
		Connection con=null;
		Statement stmt=null;
		try{
			//连接数据库
			...
			stmt=con.createStatement();
			String sql="update userinfo set userrank='"+user.getRank()+"'where userid='"+user.getUserID()+"'";
			stmt.execute(sql);
		}catch (Exception ex){
			Logger.getLogger(LoginPanel.class.getName()).log(
					Level.SEVERE, null, ex);
